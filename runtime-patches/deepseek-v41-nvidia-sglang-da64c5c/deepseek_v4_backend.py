    ...
    def init_forward_metadata_indexer(
        ...
        *,
        ...
        return PagedIndexerMetadata(
            ...
            # V4.1's c1/c2 low-ratio decode unconditionally calls the FP4
            # paged-MQA kernel.  SM120/SM121 default the ordinary FP8 c4
            # indexer to a Torch fallback, whose metadata is None; force the
            # DeepGEMM schedule for these FP4 low-ratio sources even when the
            # separate c4 FP4-indexer flag is disabled.
            force_deep_gemm_metadata=(
                get_platform().is_sm120
                and (
                    compress_ratio in (1, 2)
                    or self.enable_deepseek_v4_fp4_indexer
                )
            ),
            use_prefill_cuda_graph=use_prefill_cuda_graph,
            compress_ratio=compress_ratio,
