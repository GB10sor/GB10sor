...
# One-command coordinator for the four-Spark switched model boundary. It does
...
# four nodes, their two fabric rails, and the local-NVMe checkpoint path on
# each node.
...
[[ -r "$config_file" ]] || die "private switched-quads config is unreadable: $config_file"
...
case "$profile" in
  minimax-m3-nvfp4-quads-v028|minimax-m3-nvfp4-quads-v028-bounded16k)
    inventory="${QUADS_MINIMAX_M3_INVENTORY_FILE:-}"
    ;;
  *)
    die "profile is not enabled for the switched-Quads boundary: $profile"
    ;;
esac
...
export GB10_CLUSTER_VLLM_ENFORCE_EAGER="${QUADS_VLLM_ENFORCE_EAGER:-1}"
export GB10_CLUSTER_SERVE_MAX_SECONDS="${GB10_CLUSTER_SERVE_MAX_SECONDS:-${QUADS_SERVE_MAX_SECONDS:-0}}"
...
export GB10_CLUSTER_EVIDENCE_DIR="${GB10_CLUSTER_EVIDENCE_DIR:-$evidence_root/quads-switch-$profile-$run_id}"
...
(( ${#hosts[@]} == 4 )) || die 'private Quads inventory must contain exactly four hosts'

jq -e '.nodes | length == 4 and ([.[].host] | unique | length == 4)' \
  "$inventory" >/dev/null || die 'private Quads inventory must have four distinct hosts'
...
"$repo_root/scripts/cluster-vllm-acceptance.sh"
