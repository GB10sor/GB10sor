...
def safetensors_weights_iterator(
    ...
    # NVIDIA DeepSeek V4.1 Engram: the full checkpoint remains on read-only
    # NFS, while each TP rank's verified embedding rows reside on local NVMe.
    # The local sparse files intentionally do not contain the auxiliary Engram
    # tensors, so only the two embedding tensors may be read from them.
    engram_dir = os.environ.get("SGLANG_DSV41_ENGRAM_LOCAL_DIR")
    if engram_dir:
        if disable_mmap or prefetch:
            raise RuntimeError("local Engram requires mmap and checkpoint prefetch disabled")
        engram_dir = os.path.realpath(engram_dir)
        expected = {
        ...
        if not os.path.isdir(engram_dir):
            raise RuntimeError("local Engram directory is missing")
    ...
    for st_file in tqdm(
        ...
        if disable_mmap:
            ...
            with safetensors.safe_open(st_file, framework="pt", device="cpu") as f:
                local_names = expected.get(os.path.basename(st_file), set()) if engram_dir else set()
                if local_names:
                    local_file = os.path.join(engram_dir, os.path.basename(st_file))
                    with safetensors.safe_open(local_file, framework="pt", device="cpu") as local_f:
                        if not local_names.issubset(set(f.keys()) & set(local_f.keys())):
                            raise RuntimeError("Engram tensor names differ from NVIDIA source")
                        for name in f.keys():
                            if name in local_names:
                                if f.get_slice(name).get_shape() != local_f.get_slice(name).get_shape():
                                    raise RuntimeError("Engram tensor shape differs from NVIDIA source")
                                yield name, local_f.get_tensor(name)
                            else:
