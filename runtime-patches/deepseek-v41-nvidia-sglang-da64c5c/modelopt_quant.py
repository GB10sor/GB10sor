...
def _pad_nvfp4_cutlass_gated_weights(
    w13_weight: torch.Tensor,
    w13_weight_scale: torch.Tensor,
    w2_weight: torch.Tensor,
    w2_weight_scale: torch.Tensor,
) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
    """Keep gate/up halves aligned when CUTLASS's 128-row scale tile pads TP8."""
    original_half = w13_weight.shape[1] // 2
    assert w13_weight.shape[1] == 2 * original_half
    assert w13_weight_scale.shape[1] == 2 * original_half
    assert w2_weight.shape[2] * 2 == original_half
    assert w2_weight_scale.shape[2] * 16 == original_half
    padded_total = ((2 * original_half + 127) // 128) * 128
    pad_each = padded_total // 2 - original_half
    if not pad_each:
        return w13_weight, w13_weight_scale, w2_weight, w2_weight_scale
    assert pad_each > 0 and pad_each % 16 == 0

    def pad_halves(value: torch.Tensor) -> torch.Tensor:
        first, second = value.split(original_half, dim=1)
        return torch.cat(
            (
                torch.nn.functional.pad(first, (0, 0, 0, pad_each)),
                torch.nn.functional.pad(second, (0, 0, 0, pad_each)),
            ),
            dim=1,
        ).contiguous()

    return (
        pad_halves(w13_weight),
        pad_halves(w13_weight_scale),
        torch.nn.functional.pad(w2_weight, (0, pad_each // 2)),
        torch.nn.functional.pad(w2_weight_scale, (0, pad_each // 16)),
    )
...
class ModelOptNvFp4FusedMoEMethod(FusedMoEMethodBase):
    ...
    def process_weights_after_loading(self, layer: torch.nn.Module) -> None:
        ...
        if (
            ...
            if self.enable_flashinfer_cutlass_moe and layer.moe_runner_config.is_gated:
                # TP8 gives this checkpoint an intermediate width of 288 per
                # gated half.  swizzle_blockscale pads the combined 576 rows
                # to 640.  Padding only the end of W13 would move the gate/up
                # split, so preserve each half's boundary and pad W2's input
                # dimension to the same 320-wide half before swizzling.
                padded = _pad_nvfp4_cutlass_gated_weights(
                    layer.w13_weight,
                    layer.w13_weight_scale,
                    layer.w2_weight,
                    layer.w2_weight_scale,
                )
                if padded[0] is not layer.w13_weight:
