...
flock() { return 0; }
findmnt() {
  if [[ "$*" == *OPTIONS* ]]; then printf '%s\n' "${mount_options:-rw,relatime}"
  elif [[ "$*" == *SOURCE* ]]; then printf '%s\n' "${mount_source:-/dev/nvme0n1p2}"
  else printf '%s\n' "${filesystem:-ext4}"
  fi
}
...
new_case() {
  ...
  cp "$repo/scripts/cluster-weight-hash-shared.sh" "$state/hash.sh"
  printf '%s\n' "$token" >"$state/owner"
...
run_shared() {
  bash "$state/hash.sh" "$1" "$state" "$unit" "$token" /model "$token" 60 \
    read-only-shared server:/models
}
record_attempt() {
...
fixture_code=$(sed -n '/^cgroup_empty() {/,/^}/p' "$repo/scripts/cluster-weight-hash-shared.sh")
...
unset filesystem
new_case shared-pass; filesystem=nfs4; mount_source=server:/models; mount_options=ro,hard; export filesystem mount_source mount_options
printf '%s\n' "$token" >"$state/tree.sha256"; run_shared start; [[ -e "$state/started" && -e "$state/stopped" ]]; echo 'PASS exact read-only shared source accepted'
unset filesystem mount_source mount_options
new_case shared-rw; filesystem=nfs4; mount_source=server:/models; mount_options=rw,hard; export filesystem mount_source mount_options
printf '%s\n' "$token" >"$state/tree.sha256"; if run_shared start; then exit 1; fi; [[ ! -e "$state/started" ]]; echo 'PASS read-write shared source rejected'
unset filesystem mount_source mount_options
new_case shared-wrong-source; filesystem=nfs4; mount_source=other:/models; mount_options=ro,hard; export filesystem mount_source mount_options
printf '%s\n' "$token" >"$state/tree.sha256"; if run_shared start; then exit 1; fi; [[ ! -e "$state/started" ]]; echo 'PASS changed shared source rejected'
unset filesystem mount_source mount_options
