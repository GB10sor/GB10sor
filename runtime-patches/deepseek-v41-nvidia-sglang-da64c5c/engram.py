...
class _HostTable:
    ...
    def __init__(self, layout: str, nbytes: int, name: str, group, pin: bool):
        ...
        # GB10 can spend many minutes compacting memory synchronously for huge
        # pages. Fault ordinary pages first, then let khugepaged coalesce them
        # in the background. The table bytes and lookup semantics are unchanged.
        late_hugepage = os.environ.get("SGLANG_DSV41_ENGRAM_LATE_HUGEPAGE") == "1"
        if not late_hugepage:
        ...
        if layout == "private":
            ...
            if late_hugepage:
                self.mm.madvise(mmap.MADV_HUGEPAGE)
            if os.environ.get("SGLANG_DSV41_ENGRAM_MLOCK") == "1":
                libc = ctypes.CDLL(None, use_errno=True)
                libc.mlock.argtypes = (ctypes.c_void_p, ctypes.c_size_t)
                if libc.mlock(self.bytes.data_ptr(), nbytes) != 0:
                    raise RuntimeError(
                        f"engram host table mlock({nbytes} bytes) failed: "
    ...
    def finish_load(self, label: str):
        ...
        if (
            ...
            and os.environ.get("SGLANG_DSV41_ENGRAM_LATE_HUGEPAGE") != "1"
        ):
